...
export default function OnlineMatch({ code, role, settings, onExit }: Props) {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('broadcast', { event:'game' }, ({ payload }) => {
        ...
        if (msg.type === 'rematch') {
          setMe(createPlayer(settings.playerName.trim() || 'Spieler 1')); setOpponent(createPlayer('Gegner')); roundRef.current = 1; advancingRef.current = false; setRound(1); setFinished(false); setAdvancing(false); setStarted(true)
        }
        if (msg.type === 'state') {
          // Ignore delayed packets from an already completed round.
  ...
  useEffect(() => {
    if (!started || !me.submitted || !opponent.submitted || advancingRef.current || finished) return
    ...
    const timer = window.setTimeout(() => {
      if (roundRef.current >= TOTAL_ROUNDS) {
        ...
        setMe(p => resetRound(p))
        setOpponent(p => resetRound(p))
        setRound(nextRound)
      }
  ...
  }, [started, me.submitted, opponent.submitted, finished])
  ...
  const rematch = () => { setMe(createPlayer(settings.playerName.trim() || 'Spieler 1')); setOpponent(createPlayer('Gegner')); roundRef.current = 1; advancingRef.current = false; setRound(1); setFinished(false); setAdvancing(false); void send({type:'rematch',sender:clientId}) }
